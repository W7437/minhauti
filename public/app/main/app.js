...
function setup(){
  ...
  document.getElementById("sedationSelect")?.addEventListener("change",()=>updateNeuroVisibility(true));
...
function parseAuxiliarNursingText(raw){
  ...
  // Diurese: aceita variações comuns do Auxiliar e de texto copiado do prontuário,
  // por exemplo: "DU 825 mL", "DU: 825ml", "DU = 825 mL" ou "Diurese 825 mL".
  const duMatch=text.match(/(?:\bDU\b|\bDIURESE\b)\s*[:=\-]?\s*(\d+(?:[.,]\d+)?)\s*(?:mL|ml)?\b/i);
  if(duMatch){
    const v=Number(duMatch[1].replace(",","."));
    if(Number.isFinite(v)&&v>=0)result.du=v;
  }

  const bhMatch=text.match(/(?:\bBH\b|\bBALAN[CÇ]O(?:\s+H[IÍ]DRICO)?\b)\s*[:=]?\s*([+-]?\s*\d+(?:[.,]\d+)?)\s*(?:mL|ml)?\b/i);
  if(bhMatch){
...
function updateNeuroVisibility(updateExam=true){
  ...
  const sedated=(form.elements.sedation?.value||"")==="yes";

  document.getElementById("glasgowWrap")?.classList.toggle("hidden",sedated);
  document.getElementById("rassWrap")?.classList.toggle("hidden",!sedated);
...
function buildNeuroText(data=getData()){
  const parts=[];
  const sedated=data.sedation==="yes";

  if(sedated){
    const rass=num(data,"rass");
    if(rass!==null && rass>=-5 && rass<=4)parts.push(`RASS ${rass>0?"+":""}${rass}`);
  }else{
    const gcs=num(data,"gcs");
    if(gcs!==null && gcs>=3 && gcs<=15)parts.push(`Glasgow ${gcs}`);
  }
