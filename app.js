...
function updateRespSupportVisibility(updateExam=true){
  const oxygen=form.elements.resp_support?.value==="oxygen";
...
function generateEvolution(){
  ...
  // --- Dieta no bloco superior; HD passa para a narrativa ---
  const diagnosesText=(d.diagnoses_text||"").trim();
  if(diagnosesText){
    narrative.push(`HD: ${diagnosesText.replace(/\n+/g," / ")}.`);
  }

  const dietBlock="#Dieta: "+(d.diet||"-");
