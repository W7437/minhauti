...
function glyTempNarrative(data=getData()){
  const temp=summarizeTemperature(data).text;
...
function extractNursingData(){
  ...
  if(!raw){
    ...
    status.textContent="Cole primeiro o resumo gerado pelo Auxiliar.";
    return;
  }

  const parsed=parseAuxiliarNursingText(raw);
  const selected={
    hgt:!!document.getElementById("importHgt")?.checked,
    temp:!!document.getElementById("importTemp")?.checked,
    du:!!document.getElementById("importDu")?.checked,
    bh:!!document.getElementById("importBh")?.checked
  };

  const targets=[];
  if(selected.hgt&&parsed.hgt.length)targets.push(["hgt_series",parsed.hgt.join("-")]);
  if(selected.temp&&parsed.temp.length)targets.push(["temp_series",parsed.temp.map(v=>String(v).replace(".",",")).join("-")]);
  if(selected.du&&parsed.du!==null)targets.push(["urine_volume",parsed.du]);
  if(selected.bh&&parsed.bh!==null)targets.push(["fluid_balance",parsed.bh]);

  if(!targets.length){
    status.className="result warn";
    status.textContent="Nenhum dos dados selecionados foi reconhecido no texto.";
    return;
  }

  const populated=targets.filter(([name])=>{
    const el=form.elements[name];
    return el&&String(el.value??"").trim()!=="";
  });

  if(populated.length){
    const names=populated.map(([name])=>({
      hgt_series:"HGT",
      temp_series:"temperatura",
      urine_volume:"diurese",
    ...
    if(!confirm(`Já existem valores preenchidos para: ${names}. Deseja substituí-los pelos dados do Auxiliar?`)){
      ...
      status.textContent="Importação cancelada; os valores atuais foram mantidos.";
      return;
    }
  }
  ...
  // O resumo do Auxiliar é de 24 h por padrão.
  if(selected.du&&parsed.du!==null&&form.elements.urine_hours&&!form.elements.urine_hours.value){
    form.elements.urine_hours.value=24;
  }

  // Mantém compatibilidade com a interface manual de temperatura.
  if(selected.temp&&parsed.temp.length){
    const min=Math.min(...parsed.temp),max=Math.max(...parsed.temp);
    if(max>=38){
      ...
      // Hipotermia é representada pela série de temperaturas; não marcar "afebril".
  ...
  const imported=[];
  if(selected.hgt&&parsed.hgt.length)imported.push(`HGT ${parsed.hgt.join("-")}`);
  if(selected.temp&&parsed.temp.length)imported.push(`T ${parsed.temp.map(v=>v.toFixed(1).replace(".",",")).join("-")} °C`);
  if(selected.du&&parsed.du!==null)imported.push(`DU ${parsed.du} mL`);
  if(selected.bh&&parsed.bh!==null)imported.push(`BH ${parsed.bh>0?"+":""}${parsed.bh} mL`);

  status.className="result ok";
  status.textContent="Extraído: "+imported.join(" | ")+".";
}
