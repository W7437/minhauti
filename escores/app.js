...
function recommendFromContext(diagText,alerts,bed){
  const text=normalizeText([diagText,...(alerts||[]).map(x=>x.text||x)].join(" "));
  const rec=new Map();
  const add=(id,reason)=>{
    if(!byId(id))return;
    if(!rec.has(id))rec.set(id,[]);
    if(reason&&!rec.get(id).includes(reason))rec.get(id).push(reason);
  };
  const has=(...terms)=>terms.some(t=>text.includes(normalizeText(t)));

  if(has("sepse","choque septico","sepsis","infeccao grave")){
    add("sofa","Hipótese/alerta compatível com infecção grave ou sepse.");
    add("qsofa","Pode auxiliar na avaliação rápida em suspeita de infecção.");
    add("news2","Útil para quantificar deterioração fisiológica.");
  }
  if(has("pneumonia","pac","bronco pneumonia","broncopneumonia")){
    add("curb65","Hipótese de pneumonia/PAC reconhecida.");
    add("news2","Pneumonia pode cursar com deterioração fisiológica.");
  }
  if(has("tep","embolia pulmonar","tromboembolismo pulmonar")){
    add("wells_pe","Hipótese de TEP reconhecida.");
    add("perc","Pode ser pertinente apenas se a probabilidade pré-teste for baixa.");
    add("spesi","Útil para prognóstico após confirmação de TEP.");
  }
  if(has("dor toracica","sindrome coronariana","sca","nstemi","angina instavel","iam","infarto")){
    add("heart","Hipótese/sintoma compatível com síndrome coronariana ou dor torácica.");
    add("timi_nstemi","Pertinente em angina instável/NSTEMI.");
  }
  if(has("pancreatite")){
    add("bisap","Hipótese de pancreatite aguda reconhecida.");
  }
  if(has("tce","trauma craniano","rebaixamento","coma","neurolog","glasgow")){
    add("gcs","Contexto neurológico/rebaixamento reconhecido.");
  }
  if(has("pam abaixo","hipotens","noradrenalina","adrenalina","dva","oligur","anur","creatinina","piora laboratorial da funcao renal","plaquet","p/f","hipox")){
    add("sofa","Checklist sugere possível disfunção orgânica/hemodinâmica.");
  }
  if(has("pam abaixo","hipotens") && has("infecc","sepse")){
    add("qsofa","Hipotensão no contexto de suspeita infecciosa.");
  }

  const gcs=n(bed?.gcs);
  if(gcs!==null&&gcs<15)add("gcs","Glasgow salvo no leito está abaixo de 15.");

  return [...rec.entries()].map(([id,reasons])=>({id,reasons}));
}
...
function inputField(name,label,type="number",opts={}){
  ...
  return `<label class="field ${opts.className||""}">${esc(label)}<input ${attrs}></label>`;
}
...
function checkField(name,label,opts={}){
  return `<label class="check-field ${opts.className||""}"><input data-field="${name}" type="checkbox"><span>${esc(label)}</span></label>`;
}
...
function scoreTemplate(id){
  switch(id){
    ...
    case "heart": return `
      ...
      ${inputField("age","Idade (anos)")}
      ${inputField("risk_count","Número de fatores de risco coronariano","number",{min:0,max:10})}
...
function renderCatalog(){
  ...
  for(const s of CATALOG){
    const group=s.area.split("/")[0].trim();
    (groups[group]??=[]).push(s);
  }
  $("catalogGroups").innerHTML=Object.entries(groups).map(([g,items])=>`
    <div class="catalog-group"><h3>${esc(g)}</h3>
      ${items.map(s=>`<button type="button" class="catalog-button" data-score="${s.id}">${esc(s.name)}<small>${esc(s.description)}</small></button>`).join("")}
    </div>`).join("");
  $("catalogGroups").querySelectorAll("[data-score]").forEach(b=>b.addEventListener("click",()=>renderScore(b.dataset.score)));

  const quick=["news2","sofa","qsofa","curb65","wells_pe","heart","bisap","gcs"];
  $("quickCatalog").innerHTML=quick.map(id=>`<button type="button" class="quick-chip" data-score="${id}">${esc(byId(id).name)}</button>`).join("");
  $("quickCatalog").querySelectorAll("[data-score]").forEach(b=>b.addEventListener("click",()=>renderScore(b.dataset.score)));
}
...
function renderContext(){
  ...
  const diag=localCtx.ctx?.diagnoses||localCtx.bed?.diagnoses_text||"";
  const alerts=Array.isArray(localCtx.ctx?.alerts)?localCtx.ctx.alerts:[];
  const recs=recommendFromContext(diag,alerts,localCtx.bed);

  $("contextSubtitle").textContent=`Leito ${localCtx.active} · sugestões geradas apenas a partir do contexto salvo localmente.`;

  if(!recs.length){
    $("contextSuggestions").innerHTML=`<div class="result muted" style="margin-top:0">Nenhuma sugestão específica foi reconhecida nas hipóteses diagnósticas ou alertas atuais. Use a busca acima.</div>`;
  }else{
    $("contextSuggestions").innerHTML=recs.map(r=>{
      const meta=byId(r.id);
      return `<div class="suggestion-wrap"><button type="button" class="suggestion-chip" data-score="${r.id}">${esc(meta.name)}</button><span class="suggestion-reason">${esc(r.reasons[0]||"")}</span></div>`;
    }).join("");
    $("contextSuggestions").querySelectorAll("[data-score]").forEach(b=>b.addEventListener("click",()=>renderScore(b.dataset.score)));
  }

  const evidence=[];
  if(diag.trim())evidence.push(`<div class="evidence-block"><strong>Hipóteses:</strong> ${esc(diag.trim())}</div>`);
  if(alerts.length)evidence.push(`<div class="evidence-block"><strong>Checklist:</strong><br>${alerts.map(a=>`• ${esc(a.text||a)}`).join("<br>")}</div>`);
  if(!evidence.length)evidence.push("Nenhuma hipótese/checklist sincronizado. Salve o leito ou entre neste módulo pela barra lateral do Minha UTI.");
  $("contextEvidence").innerHTML=evidence.join("");
}
...
function setup(){
  ...
  renderContext();
  setupSearch();
  $("refreshContextBtn").addEventListener("click",renderContext);
}
if(typeof document!=="undefined")document.addEventListener("DOMContentLoaded",setup);

if(typeof module!=="undefined"&&module.exports){
  module.exports={calcNEWS2,calcQSOFA,calcSOFA,calcCURB65,calcSPESI,calcWellsPE,calcPERC,calcHEART,calcTIMI,calcBISAP,calcGCS,calcSIRS,recommendFromContext};
}
